...
const { handleGroupTextSend, handleGroupTextDelete, handleCreateGroupTextReaction, handleGetGroupTextReactions } = require('./handle-text');
    ...
    handleSocketConnection() {
        this.io.on("connection", socket => {
            ...
            socket.on('connection-made', (data) => __awaiter(this, void 0, void 0, function* () {
                ...
                const talkWithDest = yield GroupUser.findOne({
                    userId: user.userId,
                ...
                destConnection = yield GroupUser.findOne({
                    userId: data.destUser.userId,
                });
                if (!destConnection) {
                    // destConnection = await new TextTalk({
                    destConnection = yield new GroupUser({
                        userId: data.destUser.userId,
                        talk: {
                ...
                const talkWithUser = yield GroupUser.findOne({
                    userId: data.destUser.userId,
                ...
                textTalkOne = yield GroupUser.findOne({
                    userId: user.userId,
                    'talk.pairId': `${user.userId}-${data.destUser.userId}`
                });
                ...
                if (this.activeGroups.length > 0) {
                    ...
                    for (const group of this.activeGroups) {
                        ...
                        if (group && group.members && group.members.length > 0) {
                            const isUserInMember = group.members.find((element) => {
                                return element.userId === user.userId;
                            });
                            // console.log('isUserInMember', isUserInMember);
                            withoutUserMemberList = group.members.filter((element) => {
                                return element.userId !== user.userId;
                            });
            ...
            socket.on('user-info', (data) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (!socketConnection && !data.userId.startsWith('na-')) {
                    ...
                    const groupConnectoin = yield new GroupConnection({
                        userId: data.userId,
                        socketId: socket.id,
                        connectAt: data.connectAt,
                    });
                    yield groupConnectoin.save();
                }
                else {
                    // const findSocket = await TextTalk.findOne({
                    const findSocket = yield GroupUser.findOne({
                        userId: data.userId,
            ...
            socket.on('join-group', (data) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    const joinedGroup = yield GroupTalk.findOne({
                        _id: data.groupRoomId
                    });
                    const isUserInIds = joinedGroup.allMemberUserIds.find((element) => {
                    ...
                    let userExist;
                    if (group) {
                        userExist = group.members.find((member) => {
                    ...
                    if (group && !userExist) {
                        ...
                        socket.join(group.groupRoomId, () => __awaiter(this, void 0, void 0, function* () {
                            const userWithRooms = {
                                socketId: user.socketId,
                                userId: user.userId,
                                userName: user.userName,
            ...
            //   }
            // });
            socket.on('get-group-list', (data) => __awaiter(this, void 0, void 0, function* () {
                console.log('get-group-list data', data);
                const groupList = yield GroupTalk.find({});
                const returnList = [];
