...
const s3 = new aws.S3();
const deleteOldMediaFiles = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        console.log('old-videos-deletion-start');
        //// loop text talk and delete old videos
        const limit = 1000 * 60 * 60 * 24 * 31;
        // const limit = 1000*60;
        const allTextTalks = yield TextTalk.find();
                ...
                for (const userTalk of userTalks) {
                    ...
                    if (textList && textList.length > 0) {
                        for (const text of textList) {
                            ...
                            const videoPaths = fileUrls.filter((path) => {
                            ...
                            if (!process.env.S3NOTUSE && videoPaths.length > 0) {
                                ...
                                yield s3DeleteMany(deleteParam);
                            }
                            if (process.env.S3NOTUSE && videoPaths.length > 0) {
                                for (const videoPath of videoPaths) {
