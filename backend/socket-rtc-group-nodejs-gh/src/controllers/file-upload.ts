...
const fileUpload = async (req: any, res: any, next: any) => {
  try {
    ...
    //// check request user is in allMemberUserIds
    const groupTalk = await GroupTalk.findOne({ _id: req.body.joinGroupId })

    if (!groupTalk) {
      const error = new Error('groupTalk not found');
      // error.statusCode = 403;
      throw error;
      // return;
    }

    const isGroupMember = groupTalk.allMemberUserIds.find((element: any) => {
      return element.userId === req.userId;
    });

    if (!isGroupMember) {
      const error = new Error('user not found in allMemberUserIds');
      // error.statusCode = 403;
      throw error;
      // return;
    }

    // console.log(req.body.joinGroupId, isGroupMember);
